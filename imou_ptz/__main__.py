...
def _camera_args(parser):
    ...
    parser.add_argument("--port", type=int, default=int(os.environ.get("CAMERA_DVRIP_PORT", "37777")))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="IMOU/Dahua ONVIF PTZ Proxy for Frigate")
    sub = parser.add_subparsers(dest="command")

    serve_p = _camera_args(sub.add_parser("serve", parents=[parent], help="Run ONVIF PTZ proxy server"))
    serve_p.add_argument("--bind", default="0.0.0.0")
    serve_p.add_argument("--api-port", type=int, default=int(os.environ.get("API_PORT", "8000")))

    move_p = _camera_args(sub.add_parser("move", parents=[parent], help="Execute a PTZ movement"))
    ...
    if args.command == "serve":
        ...
        cam.connect()
        print(f"Moving {code_map[args.code]} speed={args.speed} duration={args.duration}s")
